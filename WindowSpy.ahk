...
WinSpyGui() {
    ...
    SetTimer "Update", 100 ; originally 250
}
...
WinGetTextFast(detect_hidden) {
    ...
    If (Type(controls) = "Array" And controls.Length) {
...
UpdateText(vCtl, NewText) {
    ...
    ; if (!oldText.HasProp(hCtl) Or OldText.%hCtl% != NewText) { ; original "if" statement
    if (oGui[vCtl].Value != NewText) {
        ctl.Value := NewText
        ; OldText.%hCtl% := NewText ; error on a117 - memory read/write error
    }
}
