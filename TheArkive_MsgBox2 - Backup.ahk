...
class msgbox2 {
	Static mRightClick := "", mContextMenu := "", mButtonPress := "", mCloseWin := ""
	Static criticalValue := A_IsCritical, sMsg := "", options := ""
	Static title := "Notification", fontSize := 8, fontFace := "Verdana", txtColor := "Default", bgColor := "Default"
	Static btnMarXr := 3, btnMarYr := 1, maxW := 350, minW := 150, forceW := false, maxH := 0
	Static icon := {x: 0, y: 0, w: 0, h: 0, file: "", num: "", hwnd: 0}
	Static btnTextW := false, selectable := false, noCloseBtn := false, btnAlign := "right"
	Static checkMsg := "",checkMsgVal := 0, dropListMsg := "", dropListMsgVal := 1, editMsg := "", editBox := false
	Static comboMsg := "", comboMsgVal := 1, listMsg := "", listVal := "", listRows := "", helpText := ""
	Static btnList := 0, btnCount := 0, btnDefault := 1, bMWd := 0, bMWc := 0
	Static helpDims := {}, btnDims := {}, btnListW := Array(), curMon := {}
	Static adjHeight := 0, adjWidth := 0, totalWidth := 0, totalHeight := 0, ctlWidth := 0
	Static hParent := 0, modal := false, parentX := "", parentY := "", parentW := "", parentH := ""
	
	__New(sMsg,title:="",options:="") {
		Critical "off"
		
		msgbox2.mRightClick := ObjBindMethod(this,"RightClick")
		mRightClick := msgbox2.mRightClick
		OnMessage(0x204,mRightClick)
		
		msgbox2.mCloseWin := ObjBindMethod(this,"CloseWin")
		mCloseWin := msgbox2.mCloseWin
		OnMessage(0x0112,mCloseWin)
		
		msgbox2.mContextMenu := ObjBindMethod(this,"ContextMenu")
		msgbox2.mButtonPress := ObjBindMethod(this,"ButtonPress")
		
		; this.criticalValue := A_IsCritical ; thanks to robodesign for this
		; this.sMsg := sMsg, this.options := options
		...
		; this.icon := {x: 0, y: 0, w: 0, h: 0, file: "", num: "", hwnd: 0} ; this.PickIcon("")
		iconHt := 32 * iconScale
	...
	Static RightClick(wParam, lParam, msg, hwnd) {
		If (hwnd = this.hwnd) {
			x := lParam & 0xff
	...
	Static CloseWin(wParam, lParam, msg, hwnd) {
		If (hwnd = this.hwnd And wParam = 0xF060) {
			this.procResult()
			this.hwnd := 0
		}
	}
	Static ContextMenu(ItemName, ItemPos, Menu) {
		clipboard := this.sMsg
	}
	Static ButtonPress(oCtl, info) {
		ctlClassNN := oCtl.ClassNN
		ctlText := oCtl.Text
		bR := Array(ctlText,ctlClassNN)
		this.procResult(bR)
	}
	Static IHKeyDown(iHook, VK, SC) {
		curKey := Format("{:X}",VK)
	...
	Static GetTextDims(r_Text, sFaceName, nHeight,maxWidth:=0) {
		; ======================================================================
